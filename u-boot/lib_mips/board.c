...
const char version_string[] = "DPTechnics U-Boot version 1.0  (" __DATE__ ", " __TIME__ ")";
...
static int init_func_ram(void){
    ...
    puts("[ERROR] RAM could not be initialized!\n");
    return 1;
}
#endif

/*
 * Begin starting the board.
 *
...
{ 
    ...
    env_init,		/* Initialize U-Boot environment */
    init_baudrate,	/* Initialize serial port settings */
    serial_init,	/* Start UART */
    console_init_f,     /* Initialize console */        
    display_banner,	/* Print first messages to the console */
    checkboard,         /* Execute some board checks */
    init_func_ram,      /* Initialize RAM */
    NULL, 
};
...
void board_init_f(ulong bootflag){
    ...
    /* Reset the global data structure */
    memset((void *)gd, 0, sizeof(gd_t));

    /* Execute initialisa sequence */
    ...
    puts("Hardware: "BOARD_CUSTOM_STRING"\n\n");

    /* Determine RAM size and report it */
     ...
     * relocate the code and continue running from DRAM. We 
     * can reserve some RAM "on top" here and round down
     * to the next 4 KiB block.
     */
...
void board_init_r(gd_t *id, ulong dest_addr){
    ...
    if(CHECK_BIT((buffer[0] & 0xFF), 0) != 0 || CHECK_BIT((buffer[0] & 0xFF), 1) != 0){
        ...
        printf("[Error] Invalid MAC, using default\n");
    }
    ...
    /* Turn on all LED's during ETH initialization */
    all_led_on();
