...
int serial_init(void){
	...
	/* Get reference clock rate, then set baud rate to 19200 */
	rdata = ar7240_reg_rd(HORNET_BOOTSTRAP_STATUS);
	rdata &= HORNET_BOOTSTRAP_SEL_25M_40M_MASK;

	if(rdata){
		baudRateDivisor = (40000000 / (16 * 19200)) - 1; // 40 MHz clock is taken as UART clock
	} else {
		baudRateDivisor = (25000000 / (16 * 19200)) - 1; // 25 MHz clock is taken as UART clock
	}
